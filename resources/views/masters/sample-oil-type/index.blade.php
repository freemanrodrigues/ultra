...
<style>
    /* Consistent styling for sample-oil-type page - Optimized for viewport */
    .sample-oil-type-page {
    ...
    .sample-oil-type-page h1 {
        font-size: 1.25rem;
        font-weight: 600;
        margin: 0;
    }
    
    .sample-oil-type-page .form-control,
    .sample-oil-type-page .form-select,
    .sample-oil-type-page .btn {
        font-size: 12px !important;
        font-weight: 500;
    }
    
    /* Bold, larger headers (14px, weight 700) for better visibility */
    .sample-oil-type-page .table th {
...
<div class="container-fluid sample-oil-type-page">
    <!-- Page Header with distinct background -->
    <div class="page-header">
        <div class="d-flex justify-content-between align-items-center">
            <h1>
                <i class="fas fa-oil-can me-2"></i> Sample Oil Type Management
                <small class="d-block d-md-inline ms-md-2 opacity-75">({{ $sample_oil_types->total() }} total)</small>
            </h1>
        </div>
    </div>
    ...
    <!-- Search and Filter Form with Add New Sample Oil Type inline -->
    <div class="card shadow-sm compact-form">
        <div class="card-body py-2">
            <form method="GET" action="{{ route('sample-oil-type.index') }}" class="row g-2 form-row-aligned">
                <div class="col-md-4">
                ...
                <div class="col-md-3">
                    <a href="{{ route('sample-oil-type.create') }}" class="btn btn-primary w-100">
                        <i class="fas fa-plus me-2"></i>
                        <i class="fas fa-oil-can me-1"></i> Add New Sample Oil Type
                    </a>
                </div>
            </form>
        </div>
                ...
                <div class="table-responsive compact-table">
                    <table class="table table-hover mb-0 table-striped table-compact">
                        <thead class="table-light">
                            <tr>
                                <th width="8%">ID</th>
                                <th width="20%">Sample Oil Type Code</th>
                                <th width="40%">Sample Oil Type Name</th>
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td>{{ str_pad($sample_oil_type->id, 5, '0', STR_PAD_LEFT) }}</td>
                                <td>
                                ...
                                <td> 
                                    @if($sample_oil_type->status == 'active')
                ...
                @if($sample_oil_types->hasPages())
                    <div class="card-footer">
                        <div class="d-flex justify-content-between align-items-center">
                            ...
                            {{ $sample_oil_types->links('pagination::bootstrap-5') }}
                        </div>
                    </div>
                @endif 
            @else
                <div class="text-center py-4">
                    ...
                    <h5 class="text-muted">No sample oil types found</h5>
                    <p class="text-muted mb-3">
                        @if(request()->hasAny(['search', 'status']))
                            No sample oil types match your search criteria.
                        @else
                            Start by adding your first sample oil type.
                        @endif
                    </p>
                    @if(request()->hasAny(['search', 'status']))
