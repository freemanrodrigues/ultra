                ...
                <div class="col-md-6">
                        ...
                        <select name="customer_id" class="form-select">
                            @foreach ($customers as $customer)
                                <option value="{{ $customer->id }}" {{ old('customer_id', $customerSiteMaster->customer_id) == $customer->id ? 'selected' : '' }}>{{ $customer->customer_name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-md-6">
                        ...
                        <option value="{{$val->id}}" {{ old('site_master_id', $customerSiteMaster->site_master_id) == $val->id ? 'selected' : '' }}>{{$val->site_name}} - {{$val->city}}</option>
                        @endforeach  
                        </select>
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="text" name="site_customer_name" value="{{ old('site_customer_name', $customerSiteMaster->site_customer_name) }}" class="form-control">
                    </div>

                    
                    <div class="col-md-6">
                        <label for="site_customer_code" class="form-label">Customer Site Code</label>
                        <input type="text" name="site_customer_code" class="form-control" value="{{ old('site_customer_code', $customerSiteMaster->site_customer_code) }}" required>
                    </div>

                    <div class="col-md-12">
                        <label for="YourPlaces" class="form-label">Address Line 1</label>
                        <input type="text" name="address" class="form-control" id="YourPlaces" value="{{ old('address', $customerSiteMaster->address) }}">
                    </div>


                    <div class="col-md-4">
                        <label for="YourCity" class="form-label">City</label>
                        <input type="text" name="city" class="form-control" id="YourCity" value="{{ old('city', $customerSiteMaster->city) }}">
                    </div>
                    ...
                    <div class="col-md-4">
                        ...
                        <select class="form-select @error('state') is-invalid @enderror" 
                            ...
                            @foreach($states as $k => $state)
                                <option value="{{ $k }}"  {{ (old('state')??$customerSiteMaster->state )== $k ? 'selected' : '' }}> {{ $state }} </option>
                            @endforeach
                        </select> 
                        @error('state')
                    ...
                    <div class="col-md-4">
                        ...
                        <select class="form-select @error('country') is-invalid @enderror" 
                            ...
                            @foreach($countries as $k => $country)
                                <option value="{{ $k }}" {{ (old('country') ?? optional($customerSiteMaster)->country ) == $k ? 'selected' : '' }}> {{ $country }} </option>
                            @endforeach
                        </select> 
                        <input type="hidden" id="YourCountryCode" name="CountryCode"/> 
                        <input type="hidden" name="lat" class="form-control" value="{{ (old('lat')??$customerSiteMaster->lat )}}" id="YourLat">
                        <input type="hidden" name="long" class="form-control" value="{{ (old('long')??$customerSiteMaster->long )}}" id="YourLong">
                    </div>
                 
                    <div class="col-md-6">
                        <label for="YourPinCode" class="form-label">PinCode</label>
                       <input type="text" id="YourPinCode" name="pincode" class="form-control"  placeholder="Your Pin Code"  value="{{ old('pincode', $customerSiteMaster->pincode) }}"/>
                    </div>
                    <div class="col-md-6">
                        <label for="status" class="form-label">Status</label>
                        <select name="status" class="form-select">
                            <option value="1" {{ old('status', $customerSiteMaster->status) == 1 ? 'selected' : '' }}>Active</option>
                            <option value="0" {{ old('status', $customerSiteMaster->status) == 0 ? 'selected' : '' }}>Inactive</option>
                        </select>
                    </div>
                </div>
