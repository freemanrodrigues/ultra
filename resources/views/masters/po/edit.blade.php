...
<div class="container-fluid po-page">
    ...
    <form method="POST" action="{{ route('po.update', $po->id) }}" id="poForm">
            @csrf
            @method('PUT')
                ...
                <div class="row g-3">
                        ...
                        <select class="form-select" id="company_id" name="company_id" required>
                            ...
                            @foreach($companies as $company)
                                <option value="{{ $company->id }}" 
                                        ...
                                        {{ $po->company_id == $company->id ? 'selected' : '' }}>
                    ...
                    <div class="col-md-6">
                        ...
                        <select class="form-select" id="site_id" name="site_id" disabled>
                            ...
                            @if($po->site_id)
                                <option value="{{ $po->site_id }}" selected>{{ $po->site->site_name ?? 'Selected Site' }}</option>
                            @endif
                        </select>
                    </div>
                    ...
                    <div class="col-md-4">
                        ...
                        <input type="text" class="form-control" id="po_number" name="po_number" 
                               placeholder="Enter PO Number" required value="{{ $po->po_number }}">
                    </div>
        ...
        <div class="card shadow-sm compact-form">
            ...
            <div class="card-body">
                <div id="samplesContainer">
                    @foreach($po->samples as $index => $sample)
                        <div class="sample-item" data-sample-index="{{ $index + 1 }}">
                            <div class="sample-item-header">
                                <h6 class="mb-0">Sample {{ $index + 1 }}</h6>
                            ...
                            <div class="row g-2">
                                <div class="col-md-4">
                                    ...
                                    <select class="form-select sample-type-select" name="samples[{{ $index + 1 }}][sample_type_id]" required onchange="onSampleTypeChange(this, {{ $index + 1 }})">
                                        <option value="">Select Sample Type</option>
                                        @foreach($sampleTypes as $sampleType)
                                            <option value="{{ $sampleType->id }}" {{ $sample->sample_type_id == $sampleType->id ? 'selected' : '' }}>
                            ...
                            @endforeach
                                ...
                                <div class="col-md-2">
                                    ...
                                    <input type="number" class="form-control sample-count" name="samples[{{ $index + 1 }}][sample_count]" 
                                           min="1" value="{{ $sample->sample_count }}" required onchange="updateSampleTotal(this, {{ $index + 1 }})">
                                </div>
                                <div class="col-md-2">
                                    <label class="form-label">Sample Rate *</label>
                                    <input type="number" class="form-control sample-rate" name="samples[{{ $index + 1 }}][sample_rate]" 
                                           step="0.01" min="0" value="{{ $sample->sample_rate }}" required onchange="updateSampleTotal(this, {{ $index + 1 }})">
                                </div>
                                <div class="col-md-2">
                                    <label class="form-label">Sample Total</label>
                                    <input type="text" class="form-control sample-total" readonly value="{{ $sample->sample_total }}">
                                </div>
                                <div class="col-md-2">
                                    <label class="form-label">Standard Test Total</label>
                                    <input type="number" class="form-control standard-test-total" name="samples[{{ $index + 1 }}][standard_test_total]" 
                                           step="0.01" min="0" value="{{ $sample->standard_test_total ?? 0 }}" onchange="updateBillingSummary()">
                                </div>
                                <div class="col-md-2">
                                    <label class="form-label">Description</label>
                                    <input type="text" class="form-control" name="samples[{{ $index + 1 }}][description]" 
                                           placeholder="Description" value="{{ $sample->description }}">
                                </div>
                            </div>
                            <div class="mt-2">
                                <label class="form-label fw-bold">Select Tests for this Sample Type:</label>
                                <div class="tests-checkbox-container mt-2" id="tests-{{ $index + 1 }}">
                                    @foreach($tests as $test)
                                        <div class="form-check mb-2">
                                            <input class="form-check-input test-checkbox" type="checkbox" 
                                                   name="samples[{{ $index + 1 }}][tests][{{ $test->id }}][test_id]" 
                                                   value="{{ $test->id }}" 
                                                   id="test_{{ $index + 1 }}_{{ $test->id }}"
                                                   data-standard-rate="{{ $test->standard_test_rate }}"
                                                   {{ $sample->tests->contains('test_id', $test->id) ? 'checked' : '' }}
                                                   onchange="updateStandardTestTotal({{ $index + 1 }}); updateBillingSummary()">
                                            <label class="form-check-label" for="test_{{ $index + 1 }}_{{ $test->id }}">
                                                <strong>{{ $test->test_name }}</strong> <span class="text-muted">(₹{{ number_format($test->standard_test_rate, 2) }})</span>
                                            </label>
                                        </div>
                                    @endforeach
                                </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    let sampleCounter = {{ $po->samples->count() }};
    ...
    // Initialize billing summary on page load
    updateBillingSummary();
});
</script>
