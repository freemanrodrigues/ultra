...
<style>
    /* Consistent styling for test page - Optimized for viewport */
    .test-page {
    ...
    .test-page .form-control,
    .test-page .form-select,
    .test-page .btn {
...
<div class="container-fluid test-page">
    ...
    <!-- Search and Filter Form with Add New Test inline -->
    <div class="card shadow-sm compact-form">
        <div class="card-body py-2">
            <form method="GET" action="{{ route('test.index') }}" class="row g-2 form-row-aligned">
                <div class="col-md-4">
                    <div class="input-group position-relative">
                        <input type="text" class="form-control" id="search" name="search" 
                               value="{{ request('search') }}" placeholder="Search by test name..." 
                               autocomplete="off">
                ...
                <div class="col-md-3">
                    <div class="d-flex gap-2">
                        ...
                        <a href="{{ route('test.index') }}" class="btn btn-outline-secondary">
                            <i class="fas fa-times"></i> Clear
                        </a>
                    </div>
                </div>
                <div class="col-md-3">
                    <a href="{{ route('test.create') }}" class="btn btn-primary w-100">
                        <i class="fas fa-plus me-2"></i>
                        <i class="fas fa-flask me-1"></i> Add New Test
                    </a>
                </div>
            </form>
        </div>
    ...
    <div id="bulk-actions" style="display: none;" class="mb-2">
        <form method="POST" action="{{ route('test.bulk_delete') }}" onsubmit="return confirm('Are you sure you want to delete selected items?')">
            @csrf
        ...
        <div class="card-body p-0">
                    ...
                    <table class="table table-hover mb-0 table-striped table-compact">
                        <thead class="table-light">
                            <tr>
                                ...
                                <th width="20%">Default Unit</th>
                                <th width="20%">Default TAT Hours</th>
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>{{ $sample_type->test_name }}</td>
                                <td>{{ $sample_type->default_unit }}</td>
                                <td>{{ $sample_type->tat_hours_default }}</td>
                                <td> 
                                ...
                                <td>
                                    <div class="btn-group" role="group">
                                        <a href="{{ route('test.show', $sample_type) }}" 
                                           class="btn btn-xs btn-outline-info" title="View">
                                            <i class="bi bi-eye"></i>
                                        </a>
                                        <a href="{{ route('test.edit', $sample_type) }}" 
                                           class="btn btn-xs btn-outline-warning" title="Edit">
                                           <i class="bi bi-pencil"></i>
                                        </a>
                                        <form action="{{ route('test.destroy',  $sample_type) }}" method="POST" 
            ...
            @else
                <div class="text-center py-4">
                    ...
                    @if(request()->hasAny(['search', 'status']))
                        <a href="{{ route('test.index') }}" class="btn btn-outline-primary me-2">
