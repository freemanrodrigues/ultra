    ...
    <div class="row justify-content-center">
        <div class="col-md-8">
            <div class="card">
                ...
                <div class="card-body">
                    <form method="POST" action="{{ route('subassembly.update',$subassembly->id) }}">
                        @csrf
                        @method('PUT')
                        <div class="mb-3">
                            ...
                            <input type="text" 
                                   ...
                                   value="{{ old('sub_assemblies_code')??$subassembly->sub_assemblies_code }}" 
                                   placeholder="Enter unique Sub Assembly code (e.g., Sub Assembly001)"
                                   maxlength="50"
                                   required>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" 
                                   ...
                                   value="{{ old('sub_assemblies_name')??$subassembly->sub_assemblies_name }}" 
                        ...
                        <div class="mb-4">
                            ...
                            <select class="form-select @error('status') is-invalid @enderror" id="status" name="status" required>
                                ...
                                <option value="1" {{ (old('status')??$subassembly->status) === 1 ? 'selected' : '' }}>Active</option>
                                <option value="0" {{ (old('status')??$subassembly->status) === 0 ? 'selected' : '' }}>Inactive</option>
                            </select>
                            @error('status')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="d-flex gap-2">
                            <button type="submit" class="btn btn-primary">
                                <i class="bi bi-check-circle"></i> Update Sub Assembly Master
                            </button>
