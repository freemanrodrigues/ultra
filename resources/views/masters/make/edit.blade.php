    ...
    <div class="row justify-content-center">
        <div class="col-md-8">
            <div class="card">
                ...
                <div class="card-body">
                    <form method="POST" action="{{ route('make.update',$make) }}">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" 
                                   ...
                                   id="make_code" 
                                   name="make_code" 
                                   value="{{ old('make_code')??$make->make_code }}" 
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" 
                                   ...
                                   id="make_name" 
                                   name="make_name" 
                                   value="{{ old('make_name')??$make->make_name }}" 
                        ...
                        <div class="mb-4">
                            ...
                            <select class="form-select @error('status') is-invalid @enderror" id="status" name="status" required>
                                ...
                                <option value="1" {{ (old('status')??$make->status)  === 1 ? 'selected' : '' }}>Active</option>
                                <option value="0" {{ (old('status')??$make->status) === 0 ? 'selected' : '' }}>Inactive</option>
                            </select>
                            @error('status')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
