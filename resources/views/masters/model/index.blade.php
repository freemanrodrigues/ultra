...
<div class="container mt-4">
 <!-- Search and Filter Form --><div class="d-flex justify-content-between align-items-center mb-4">
    <h1 class="h3 mb-0">
        ...
        <small class="text-muted">({{ $models->total() }} total)</small>
    </h1>
            ...
            <div class="table-responsive">
                <table class="table table-hover mb-0">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>
                                <input type="checkbox" name="selected_ids[]" value="{{ $model->id }}" 
                            ...
                            <td>{{ $model->model }}</td>
                            <td>
                                <span class="badge status-badge {{ $model->status == 'active' ? 'bg-success' : 'bg-secondary' }}">
                                    <i class="fas {{ $model->status == 'active' ? 'fa-check' : 'fa-times' }}"></i>
                                    {{ ucfirst($model->status) }}
                                </span>
                            </td>
                            <td>
                                <small class="text-muted">@if(!empty($model->created_at)){{ $model->created_at->format('M d, Y') }} @endif</small>
                            </td>
                            <td>
